...
Де лежать зібрані пакети. Цей репозиторій публічний, тому в його релізі `course-backup-2026-09` і в
маніфесті сайту лише варіант `--no-control`: підсумковий тест там без питань. Пакет із контрольним банком
завантажується тільки в реліз приватного репозиторію `operatsiinyi-menedzhment-control`.
...
(`src/engines/progress/scorm-store.ts`). Модуль у курсі створює `om_create_scorm()` (`lib/content.php`, константа `GRADEHIGHEST` — з `mod/scorm/locallib.php`, який цей файл підключає сам):
...
**Які тренажери йдуть у Moodle пакетом (рішення 2026-09-23).** Пакет SCORM є лише в матриці p02. Для 15
розрахункових тренажерів практичних p01, p03–p07 пакети свідомо не робляться: тренажери формативні (у журналі
вони й так мали б вагу 0), оцінку за практичну ставить викладач у Завданні за рубрикою, а 15 окремих пакетів —
це 15 копій рушія з власним життєвим циклом у Moodle. У курсі розрахункові тренажери йдуть посиланням на сайт:
опис Завдання кожної практичної з тренажерами веде на її сторінку `praktychni/<id>/`. Якщо колись знадобиться
бал тренажера в журналі Moodle — пакет додається за контрактом `src/engines/README.md`, розділ «Як додати
тренажер», п. 6.
